# EC2NodeClass for Airflow workloads (ARM64 AL2023)
resource "kubernetes_manifest" "karpenter_ec2nodeclass_airflow" {
  manifest = {
    ...
    spec = {
      ...
      # Tags for Airflow workload identification
      tags = merge(var.additional_tags, {
        Name                     = "karpenter-airflow-node"
        "workload"               = "airflow"
        "karpenter.sh/discovery" = var.cluster_name
